...
create table public.citas (
  ...
  -- Una cita puede tener varios obsequios/cortesías a la vez.
...
begin
  ...
  -- Antes (mientras se puede reprogramar) la profesional se asigna o cambia,
  -- y lo que se le va a hacer a la clienta sigue ajustable -- ver más abajo.
  ...
  -- HACER (servicios, adicional, obsequios, nota) sigue ajustable hasta que la
  -- atiendan: una clienta puede pedir un servicio más días después de agendar,
  -- y antes eso obligaba a cancelar la cita y volver a agendarla perdiendo el
  -- abono ya registrado.
...
-- Borrar una cuenta de clienta (no de personal -- borrar personal se presta
-- a perder de vista quién hizo qué en el historial de trabajos/ventas ya
-- hechos, así que se deja fuera a propósito; para eso ya existe "Sin
-- acceso"). Borra auth.users, que en cascada se lleva profiles. Si la
-- clienta ya tiene citas o créditos registrados, la base de datos rechaza
-- el borrado por la FK (no hay on delete cascade ahí a propósito, para no
-- perder ese historial en silencio) -- el frontend lo traduce a un mensaje
-- claro y sugiere quitarle el acceso en su lugar.
...
create index idx_gastos_salon on public.gastos(salon_id);
...
create index idx_consignaciones_salon on public.consignaciones(salon_id);
create index idx_consignaciones_fecha on public.consignaciones(fecha);
